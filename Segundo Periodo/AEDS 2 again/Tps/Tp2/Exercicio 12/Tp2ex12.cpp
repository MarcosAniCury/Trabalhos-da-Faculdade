...
void bolha(Jogador *NBA, int n)
{
    ...
    for (i = (n - 1); i > 0; i--) 
    {
        for (j = 0; j < i; j++) 
        {
            if (NBA[j].anoNascimento >= NBA[j + 1].anoNascimento)
            {
                if(NBA[j].anoNascimento > NBA[j+1].anoNascimento || strcmp(NBA[j].nome, NBA[j+1].nome) > 0)
                {
                    Jogador temp = clone(NBA[j]);
                    NBA[j] = clone(NBA[j+1]);
                    NBA[j+1] = temp;
                }
            }
