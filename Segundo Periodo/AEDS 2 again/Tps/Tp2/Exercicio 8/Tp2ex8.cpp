...
void insercaoPorCor(Jogador *NBA, int n, int cor, int h)
{
    for (int i = (h + cor); i < n; i+=h) 
    {
        ...
        int j = i - h;
        bool flag = true;
        while ((j >= 0) && (NBA[j].peso >= tmp.peso) && flag) 
        {
            ncomparacoes++;
            if ((NBA[j].peso == tmp.peso && strcmp(NBA[j].nome, tmp.nome) > 0) || NBA[j].peso > tmp.peso)
            {
                NBA[j + h] = NBA[j];
                j-=h;
                nmovimentacoes++;
