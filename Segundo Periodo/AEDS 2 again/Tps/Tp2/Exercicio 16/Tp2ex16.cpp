...
void Insecao(Jogador* NBA, int nNBA)
{
    for (int i = 1; i < nNBA; i++) 
    {
        Jogador tmp = clone(NBA[i]);
        int j = i - 1;
        
        bool flag = true;
        while ( (j >= 0) && (NBA[j].anoNascimento >= tmp.anoNascimento && flag))
        {
            ncomparacoes++;
            if (NBA[j].anoNascimento == tmp.anoNascimento && strcmp(NBA[j].nome,tmp.nome) > 0)
            {
                ncomparacoes += 2;
                NBA[j + 1] = clone(NBA[j]);
                j--;
            ...
            else if(NBA[j].anoNascimento > tmp.anoNascimento)
            {
                ncomparacoes += 2;
                NBA[j + 1] = clone(NBA[j]);
                j--;
...
{
    FILE* arq;
    arq = fopen("matricula_insercao.txt", "w");
