...
public class Tp2ex13
{
    ...
    public static Jogador[] intercalar(int esq, int meio, int dir, Jogador[] NBA)
    {
        int n1, n2, i, j, k;
        ...
        Jogador[] a1 = new Jogador[1+n1];
        Jogador[] a2 = new Jogador[1+n2];
  
        //Inicializar primeiro subarray
        for(i = 0; i < n1; i++)
           a1[i] = NBA[esq+i].clone();
  
        //Inicializar segundo subarray
        for(j = 0; j < n2; j++)
           a2[j] = NBA[meio+j+1].clone();
        ...
        for(i = j = 0, k = esq; k <= dir; k++)
           NBA[k] = (a1[i].Getuniversidade().compareTo(a2[j].Getuniversidade()) <= 0) ? a1[i++].clone() : a2[j++].clone();

        return NBA.clone();
    }

    private static Jogador[] mergesort(int esq, int dir, Jogador[] NBA) 
    {
        if (esq < dir)
        {
           int meio = (esq + dir) / 2;
           NBA = mergesort(esq, meio, NBA);
           NBA = mergesort(meio + 1, dir, NBA);
           NBA = intercalar(esq, meio, dir, NBA);
        }
        return NBA.clone();
    }
    
    public static Jogador[] sort(int nNBA, Jogador[] NBA) 
    {
        return mergesort(0, nNBA - 1, NBA);
    }
    ...
    public static void main(String [] args)
    {
        ...
        Jogador[] NBAOrdenado = sort(NBA.length,NBA);
