...
public class Tp2ex18
{
    ...
    public static Jogador[] sort(Jogador[] NBA, int nNBA) 
    {
        return quicksort(0, nNBA - 1, NBA);
    }

    public static Jogador[] quicksort(int esq, int dir, Jogador[] NBA) 
    {
        int i = esq, j = dir;
    ...
    public static void main(String [] args)
    {
        ...
        //ordenar por estado
        Jogador[] NBAOrdenado = sort(NBA,NBA.length);
        int i = 0;
        int contordena = 0;
        while (i < k)
        {
            int temp = i;
            for(;NBAOrdenado[i].GetestadoNascimento().equals(NBAOrdenado[temp].GetestadoNascimento());i++);
            Jogador[] NBAOrdenando = new Jogador[i-temp];
            for (int j = 0,x = temp;x < i;j++, x++)
                NBAOrdenando[j] = NBAOrdenado[x].clone();
            NBAOrdenando = Selecao(NBAOrdenando, NBAOrdenando.length);
            int temp2 = contordena;
            for (int j = 0;j < i - temp2;j++)
            {
                NBAOrdenado[contordena] = NBAOrdenando[j];
                contordena++;
                NBAOrdenando[j] = null;
            }
        }
