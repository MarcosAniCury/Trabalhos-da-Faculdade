...
class ArvoreBinaria
{
    ...
    public No inserir(Jogador elemento, No i)
    {
        try
        {
            ...
            else if (elemento.Getnome().compareTo(i.elemento.Getnome()) < 0)
            {
                ...
                Tp4ex5.ncomparacoes += 2;
            }
            else if (elemento.Getnome().compareTo(i.elemento.Getnome()) > 0)
            {
                i.dir = inserir(elemento,i.dir);
                Tp4ex5.ncomparacoes += 3;
            }
            else
    ...
    public void pesquisa(String[] nomes)
    {
        pesquisa(raiz,nomes);
    }

    public void pesquisa(No i,String[] nomes)
    {
        if (i != null)
        {
            pesquisa(i.esq,nomes);
            for(int j = 0;j < nomes.length;j++)
            {
                String nome = i.elemento.Getnome();
                if (nome.startsWith(nomes[j]))
                {
                    MyIO.println(i.elemento.Getnome());
                    break;
                }
            }
            pesquisa(i.dir,nomes);
        }
    }
...
public class Tp4ex5
{
    ...
    {
        File log = new File("matricula_treesort.txt");
        log.createNewFile();
