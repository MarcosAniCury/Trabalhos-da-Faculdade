...
class Selecao
{
    ...
    public Selecao (Personagem[] array, int n)
    {
        this.array = array;
        this.n = n;
        selecao();
    ...
    public static void selecao() 
    {
        for (int i = 0; i < (n - 1); i++) 
        {
            String menor = array[i].Getsnome();
            int posicao = i;
            for (int j = (i + 1); j < n; j++)
            {
                if (menor.compareTo(array[j].Getsnome()) >= 0)
                {
                    menor = array[j].Getsnome();
                    posicao = j;
                }
            }
            swap(posicao, i);
...
class Lista 
{
    ...
    {
        Selecao Ordenador = new Selecao(array,n);
