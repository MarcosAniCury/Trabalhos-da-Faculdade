...
class Countingsort
{
    ...
    public Countingsort (Personagem[] array, int n)
    {
        this.array = array;
        this.n = n;
        countingsort();
    ...
    {
        int maior = array[0].Getspeso();
        for (int i = 0; i < n; i++) 
        {
            if(maior < array[i].Getspeso())
                maior = array[i].Getspeso();
    ...
    public static void countingsort() 
    {
        ...
        for (int i = 0; i < n; count[array[i].Getspeso()]++, i++);
  
        for(int i = 1; i < count.length; count[i] += count[i-1], i++);
  
        for(int i = n-1; i >= 0; ordenado[count[array[i].Getspeso()]-1] = array[i], count[array[i].Getspeso()]--, i--);
...
class Lista 
{
    ...
    {
        Countingsort Ordenador = new Countingsort(array,n);
